...
exports.encodeShare = encodeShare;
exports.decodeShare = decodeShare;
exports.splitWithVerify = splitWithVerify;
exports.recoverFromWires = recoverFromWires;
...
function decodeShare(wire) {
    const parts = wire.trim().split('-');
    if (parts.length !== 7)
        throw new Error('bad share format: expected 7 fields');
    ...
    if (threshold < 2 || total < threshold)
        throw new Error('bad threshold/total');
    ...
    if (gotCrc !== wantCrc)
        throw new Error('checksum mismatch — QR misread or corrupted share');
 ...
 */
function splitWithVerify(secret, threshold, total, gen, randomBytes) {
    const shares = (0, shamir_1.split)(secret, threshold, total, gen, randomBytes);
    ...
    for (const subset of subsets) {
        const rec = (0, shamir_1.combine)(subset);
 ...
 */
function recoverFromWires(wires) {
    if (wires.length < 2)
    ...
    for (const s of shares) {
        if (s.gen !== gen)
            throw new Error(`stale share: generation ${s.gen} != ${gen}. Re-scan same-generation shares.`);
    }
    if (shares.length < threshold) {
        throw new Error(`need ${threshold} shares to recover, scanned ${shares.length}`);
    }
    return (0, shamir_1.combine)(shares);
}
