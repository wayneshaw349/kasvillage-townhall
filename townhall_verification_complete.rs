...
pub fn generate_stats_proof(witness: &StatsWitness) -> Result<StatsProof, String> {
    ...
    let avg_completion_time_ms = if witness.completed > 0 {
        witness.total_completion_time_ms / witness.completed
    } else { 0 };
    
    let days_since_last_deadlock = if witness.last_deadlock_timestamp > 0 {
        let now = current_timestamp();
        Some((now - witness.last_deadlock_timestamp) / 86400)
    } else { None };
    ...
    if let Some(_pk) = STATS_PK.as_ref() {
        ...
        proof_data.extend_from_slice(&witness.total_completion_time_ms.to_le_bytes());
        ...
        proof_data.extend_from_slice(&witness.last_deadlock_timestamp.to_le_bytes());
        ...
        Ok(StatsProof {
            ...
            public_inputs: StatsPublicInputs {
                ...
                last_deadlock_ms: if witness.last_deadlock_timestamp > 0 { 
                    Some(witness.last_deadlock_timestamp * 1000) 
                } else { None },
...
pub async fn aggregate_and_prove_stats(pubkey: &str) -> Result<(CounterpartyStats, StatsProof), String> {
    ...
    let deadlock_history = DeadlockStats {
        ...
        days_since_last_deadlock: if l1_stats.last_deadlock_daa > 0 {
            Some(current_daa.saturating_sub(l1_stats.last_deadlock_daa) / DAA_PER_DAY)
        } else {
            None
        },
        unique_counterparties_deadlocked: l1_stats.unique_counterparties_deadlocked.len() as u64,
...
async fn aggregate_stats_from_frost_events(pubkey: &str) -> Option<CounterpartyStats> {
    ...
    let deadlock_history = DeadlockStats {
        ...
        resolved_after_deadlock: 0,
        last_deadlock_daa: l1_stats.last_deadlock_daa,
        daa_since_last_deadlock: l1_stats.last_deadlock_daa.map(|d| current_daa.saturating_sub(d)),
