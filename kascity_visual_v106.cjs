// kascity_visual_v106.cjs
// Reads showcase_kascity103.html -> showcase_kascity106.html   (patches the embedded scene JSON)
// Supersedes v104/v105: guard is human-only and buy-phase-only from the start.
...
const guard = q('{"sequence":[{"cond":"world.flags.phase == 2 && world.flags.buy >= 0 && seat() <= world.flags.humans && world.flags.buy_turn != world.flags.turn"},' +
...
fs.writeFileSync('showcase_kascity106.html', html);
console.log('OK showcase_kascity106.html (' + (fs.statSync('showcase_kascity106.html').size/1024/1024).toFixed(1) + ' MB)');
