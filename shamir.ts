...
export interface ShamirShare {
  index: number;      // x-coordinate, 1..255, unique per share
  gen: number;        // generation counter (for re-split / staleness)
  threshold: number;  // K — how many shares needed
  total: number;      // N — how many shares issued
  data: Uint8Array;   // y-values, one per secret byte
}
...
export function combine(shares: ShamirShare[]): Uint8Array {
  ...
  for (const s of shares) {
    if (s.gen !== gen) throw new Error(`generation mismatch: share gen ${s.gen} != ${gen} (stale share)`);
  ...
  const use: ShamirShare[] = [];
  for (const s of shares) {
    if (s.index < 1 || s.index > 255) throw new Error('invalid share index');
    if (!seen.has(s.index)) {
      seen.add(s.index);
      use.push(s);
      if (use.length === threshold) break;
    }
  }
  if (use.length < threshold) throw new Error('not enough distinct shares');
...
export function sameGeneration(shares: ShamirShare[]): boolean {
