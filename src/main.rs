...
static PROHIBITED_PATTERNS: Lazy<Vec<(Regex, &'static str, Severity)>> = Lazy::new(|| {
    vec![
        ...
        (Regex::new(r"(?i)\b(buy|sell|order|purchase|ship|deliver|source|supply|score)\s+(a\s+|an\s+|some\s+)?(cocaine|crack|heroin|meth|methamphetamine|fentanyl|carfentanil|mdma|ecstasy|molly|lsd|ketamine|pcp|dmt|oxycontin|oxycodone|percocet|percs|xanax|xannies|adderall|vicodin|hydrocodone|codeine|opioids?|opiates?|shrooms|psilocybin|ghb|rohypnol|roofies)\b").unwrap(), "drug_sale", Severity::Critical),
        (Regex::new(r"(?i)\b(cocaine|crack|heroin|meth|methamphetamine|fentanyl|carfentanil|mdma|ecstasy|molly|lsd|ketamine|pcp|dmt|oxycontin|oxycodone|percocet|percs|xanax|xannies|adderall|vicodin|hydrocodone|codeine|opioids?|opiates?|shrooms|psilocybin|ghb|rohypnol|roofies)\s+(for\s+sale|available|in\s+stock|shipped|delivered)\b").unwrap(), "drug_sale_offer", Severity::Critical),
        (Regex::new(r"(?i)\b(sex\s+trafficking|labor\s+trafficking|organ\s+(sale|trade|harvest))\b").unwrap(), "trafficking_expanded", Severity::Critical),
