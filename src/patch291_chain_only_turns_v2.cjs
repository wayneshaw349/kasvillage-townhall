// patch291_chain_only_turns_v2.cjs — same as v1, correct controller anchor (10-space indent).
...
const CTL = '        if(window.KV_SETSTATE){\n' +
...
'        }';
const ASK = '          if(window.KV_SETSTATE){\n' +
'            window.KV_SETSTATE("rollshow",0); window.KV_SETSTATE("rollv",0); window.KV_SETSTATE("rollt",0);\n' +
...
need(ASK, 1, "ASK controller ask block");
...
// CTL: authorise the controller's own step writes
s = s.replace(CTL,
...
// ASK: the ask path writes no turn/seat, but mark it anyway for clarity/safety
s = s.replace(ASK,
'          if(window.KV_SETSTATE){\n' +
'            window.__KV_CTL_WRITE=1;   // __KV_V291\n' +
'            window.KV_SETSTATE("rollshow",0); window.KV_SETSTATE("rollv",0); window.KV_SETSTATE("rollt",0);\n' +
...
// D: the funnel, appended at EOF so it wraps the final KV_SETSTATE
const APPEND = `
<script>
