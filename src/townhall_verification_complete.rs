...
async fn query_arweave_frost_events(pubkey: &str) -> Result<Vec<FrostEvent>, String> {
    ...
    let query = format!(
        r#"query {{ transactions( tags: [ {{ name: "App-Name", values: ["KasVillage"] }}, {{ name: "KV-Type", values: ["frost-agreement"] }}, {{ name: "KV-Pubkey", values: ["{}"] }} ], first: 100, sort: HEIGHT_DESC ) {{ edges {{ node {{ id tags {{ name value }} block {{ timestamp }} }} }} }} }}"#,
        pubkey_hash
    );
    let client = reqwest::Client::new();
    ...
    if let Some(edges) = edges {
        for edge in edges {
            ...
            let event_type = match get_tag("KV-Status").as_deref() {
                ...
                Some("Agreed") | Some("Accepted") | Some("Signed") => FrostEventType::AgreementCreated,
            ...
            events.push(FrostEvent {
                ...
                buyer_pubkey: get_tag("KV-Pubkey").unwrap_or_default(),
                seller_pubkey: get_tag("KV-Counterparty").unwrap_or_default(),
                ...
                deadlock_reason: match get_tag("KV-DeadlockReason").as_deref() {
