// make_kascity_v3.cjs — KasCity v3. Emits kascity_v8.json + showcase_kascity8.html.
//
...
CELL.forEach(function (c, ci) {
  const kids = [
    { id: "cel_bg_" + c.id,  mesh: "cell_bg",  material: "cel_" + c.id, transform: { pos: [0, 0, -0.4] } },
    { id: "cel_fr1_" + c.id, mesh: "cell_bar", material: "ink", transform: { pos: [0, 2.55, -0.3] } },
    { id: "cel_fr2_" + c.id, mesh: "cell_bar", material: "ink", transform: { pos: [0, -2.55, -0.3] } },
    { id: "cel_sp1_" + c.id, mesh: "speed",    material: "ink", transform: { pos: [-3.1, 1.5, -0.2], rot: [0, 0, 24] } },
    { id: "cel_sp2_" + c.id, mesh: "speed",    material: "ink", transform: { pos: [3.1, -1.4, -0.2], rot: [0, 0, -24] } }
  ];
  (ICONS[c.icon] || []).forEach(function (p, k) {
    kids.push({ id: "cel_ic" + k + "_" + c.id, mesh: p[0], material: "ink",
      transform: { pos: [p[1], p[2] + 0.25, p[3]], rot: [0, 0, p[4]] } });
  });
  nodes.push({ id: "cell_" + c.id, hidden: true, tags: ["cell"],
    transform: { pos: [0, 6.5, 0], rot: [90, 0, 0] }, children: kids });
});
...
const scene = {
  kind: "kv_game_v1",
  engine: "scene",
  meta: { id: "kascity_v8", name: "KasCity", seed: "kc8", players: PLAYERS, category: "board" },
  debug: false,
  ...
  resources: {
    meshes: {
      ...
      cell_bg: { type: "box", size: [13.5, 5.4, 0.1] },
      cell_bar: { type: "box", size: [13.5, 0.34, 0.12] },
      speed: { type: "box", size: [3.4, 0.13, 0.1] },
      ic_bar: { type: "box", size: [2.3, 0.3, 0.14] },
      ic_sheet: { type: "box", size: [1.9, 1.7, 0.12] },
      ic_tube: { type: "box", size: [1.5, 0.42, 0.42] },
      ic_drop: { type: "box", size: [0.34, 0.34, 0.34] },
      ic_puff: { type: "box", size: [1.5, 0.8, 0.5] },
      ic_rain: { type: "box", size: [0.16, 0.9, 0.16] },
      ic_knob: { type: "box", size: [0.3, 0.3, 0.3] },
      ic_head: { type: "box", size: [1.25, 1.0, 0.9] },
...
fs.writeFileSync("kascity_v8.json", json);
console.log("OK kascity_v8.json (" + (json.length / 1024).toFixed(1) + " KB, BT " +
  (JSON.stringify(bt).length / 1024).toFixed(1) + " KB, nodes " + nodes.length + ")");

const engine = fs.readFileSync("scene_engine.html", "utf8");
const inject = [
  "", "// ---- injected kascity v8 showcase ----",
  "try { loadScene(" + JSON.stringify(json) + "); }",
  "catch (e) { console.error('kascity8 boot: ' + (e && e.message)); }", ""
].join("\n");
fs.writeFileSync("showcase_kascity8.html",
  engine.replace("</script>", inject + "\n</script>"));
console.log("OK showcase_kascity8.html");
