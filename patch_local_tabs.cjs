// patch_local_tabs.cjs — replace sort chips with local_agreements-backed tabs (Active/Buyer/Seller)
// Display-layer only. Background Arweave inbox untouched; its CARDS are hidden.
// Paste Buyer Proposal input + Look Up + Refresh Inbox all kept.
const fs = require('fs');
...
// EDIT 1 — add state: which tab + the local list + loader flag
...
// EDIT 2 — loader fn, placed right after loadInbox definition closes.
// Anchor: the line that flips inboxLoading false at the end of loadInbox.
const OLD2 = `    setInboxLoading(false);
  };

  
  // === TX TEMPLATE + DELAYED R FLOW ===`;
const NEW2 = `    setInboxLoading(false);
  };
  ...
  const resumeLocal = async (row: any) => {
    try {
      ...
      // poll / proposed / anything mid-ceremony
...
// EDIT 3 — load local list when the Join screen opens (alongside loadInbox)
...
// EDIT 4 — replace the sort-chip row with Active/Buyer/Seller tabs.
// The chips only render when inboxAgreements.length > 0; the tabs must render always.
...
// EDIT 5 — hide the Arweave inbox cards (logic stays; only the render is gated off).
