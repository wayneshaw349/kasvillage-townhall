// make_kascity_v3.cjs — KasCity v3. Emits kascity_v3.json + showcase_kascity3.html.
//
...
const materials = {
  ...
  bldg: { color: "#fdf6e3" }, roof: { color: "#a03d2e" }, monu: { color: "#b8a878" },
...
T.forEach((t, i) => {
  ...
  p0sel.push(seq(
    ...
    act("setState", ["oask", 1]),
    act("prompt", ["oans", "Player " + "offered ".replace("offered ", "") + "A rival offers " + price + " for " + t.n + ". Sell?", "Sell (" + price + ")", "Keep it"])
  ));
});
...
p0sel.push(seq(cond("world.flags.go >= 0"),
  act("drawCard", ["fate"]),
  act("playSound", ["dice"]),
...
for (let s = 1; s <= PLAYERS; s++) {
  for (let i = 0; i < N; i++) {
    ...
    resolve.selector.push(seq(
      ...
      actTo("teleport", "token_p" + s, [p[0], 0.2, p[2] + (s * 0.35 - 0.9)]),
...
const scene = {
  kind: "kv_game_v1",
  engine: "scene",
  meta: { id: "kascity_v3", name: "KasCity", seed: "kc3", players: PLAYERS, category: "board" },
  debug: false,
...
fs.writeFileSync("kascity_v3.json", json);
console.log("OK kascity_v3.json (" + (json.length / 1024).toFixed(1) + " KB, BT " +
  (JSON.stringify(bt).length / 1024).toFixed(1) + " KB, nodes " + nodes.length + ")");

const engine = fs.readFileSync("scene_engine.html", "utf8");
const inject = [
  "", "// ---- injected kascity v3 showcase ----",
  "try { loadScene(" + JSON.stringify(json) + "); }",
  "catch (e) { console.error('kascity3 boot: ' + (e && e.message)); }", ""
].join("\n");
fs.writeFileSync("showcase_kascity3.html",
  engine.replace("</script>", inject + "\n</script>"));
console.log("OK showcase_kascity3.html");
