...
function patchFile(F,bak,edits){
  ...
  for(const [name,anchor,repl] of edits){
    const tokens={'gocold-fn':'vaultGoCold','cold-error':'Vault is COLD','import':'vaultGoCold','button':'Go Cold (Vault)'};
    if(s.includes(tokens[name])){console.log('skip '+name);continue;}
