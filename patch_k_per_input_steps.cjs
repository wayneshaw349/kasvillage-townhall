...
function replaceFn(startAnchor, newLines, tag) {
  ...
  if (j < 0) { console.error(`SKIP [${tag}] no closing brace at column 0`); return; }
  s = s.slice(0, i) + newLines.join(NL) + s.slice(j + CLOSE.length);
  applied++; console.log(`APPLIED [${tag}]`);
}
...
// ============================ CHECKS ============================
if (applied !== 16) { console.error(`ABORT - ${applied}/16 applied, file NOT written`); process.exit(1); }
...
const post = [
  ...
  ['partialSign(nonce, template.R[i],', 2],
