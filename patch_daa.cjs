// patch_daa.cjs — timeoutN was computed as minutes*60, i.e. 1 DAA/sec.
...
// ---- post-conditions ----
if (occurrences(files[NA], '* 60)') > 0) { console.warn('NOTE: some "* 60" remain in ' + NA + ' — check they are not DAA conversions'); }
if (occurrences(files[NA], 'DAA_PER_MIN') !== 9) { console.error('ABORT: expected 9 DAA_PER_MIN refs, saw ' + occurrences(files[NA], 'DAA_PER_MIN')); process.exit(1); }
if (occurrences(files[CS], 'minRemaining') !== 4) { console.error('ABORT: expected 4 minRemaining refs, saw ' + occurrences(files[CS], 'minRemaining')); process.exit(1); }
