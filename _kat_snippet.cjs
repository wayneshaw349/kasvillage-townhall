...
function sha512(data) {
  ...
  const padded = [...data, 128];
  while (padded.length % 128 !== 112) padded.push(0);
  for (let i = 7; i >= 0; i--) padded.push(0);
...
async function pbkdf2HmacSha512(password, salt, iterations, dkLen) {
  ...
  for (let i = 1; i <= blocks; i++) {
    ...
    u1salt.set(salt);
    u1salt[salt.length] = i >> 24 & 255;
    u1salt[salt.length + 1] = i >> 16 & 255;
    u1salt[salt.length + 2] = i >> 8 & 255;
    u1salt[salt.length + 3] = i & 255;
