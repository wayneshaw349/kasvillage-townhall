    ...
    const pollBalance = async () => {
      ...
      try {
        ...
        if (balance > 0 && balance < expectedTotal && utxoCount >= 1) {
          ...
          if (role === 'buyer' && balance >= otherExpected * 0.95 && myExpected > 0) {
...
Alert.alert('Funds Released!', 'TX: ' + (result.txId || '').slice(0, 16) + '...\nFunds returned to your wallet.');
  ...
  useEffect(() => {
    const generateFrostAddress = async () => {
      if (step === 3 && !contract.multisigAddress && contract.buyerPubkey && contract.sellerPubkey) {
        ...
        const aggPk = (contract.frostData?.aggregatedPubkey || '').replace(/^02|^03/, '');
        const frostScriptCheck = aggPk.length === 64 ? '20' + aggPk + 'ac' : '';
        if (frostScriptCheck && !isPureP2PK(frostScriptCheck)) { console.error('[COVENANT] ⚠️ FROST address has non-standard script!'); await new Promise((resolve) => Alert.alert('⚠️ Covenant Detected', 'FROST address contains a non-standard script. This may be a programmed UTXO that could claw back funds.\n\nProceed at your own risk.', [{ text: 'Stop (Recommended)', style: 'cancel', onPress: () => resolve(false) }, { text: 'I understand, continue', style: 'destructive', onPress: () => resolve(true) }])).then(ok => { if (!ok) return; }); }
      ...
      laUpsert({ agrId: agreementId, role: 'buyer', origin: 'mine' }).catch(() => {});
              ...
              // R nonce is now generated at signing time (step 5), not proposal time
              // This keeps k alive for seconds instead of days
              addToFrostList({
                agrId: agreementId, frostAddr: frostData.address, role: role === 'seller' ? 'seller' : 'buyer', step: 3,
                buyerAmount: contract.itemPriceKas, sellerAmount: contract.sellerCommitmentKas,
                buyerPubkey: contract.buyerPubkey || '', sellerPubkey: contract.sellerPubkey || '',
  ...
  // === LOCAL AGREEMENTS (source of truth for the visible list) ===
...
const nextActionMsg = (ph: string, rl: string): string => {
      ...
      switch (ph) {
        ...
        case 'templates_ready': return seller ? 'Your templates are with the buyer. When their co-signature comes back, paste it into your PURPLE box - your collateral funds right after.' : 'Paste the seller templates into your AMBER box, co-sign, and send the signature back (their PURPLE box).';
    ...
    if (!(agreement as any)._kvScanned) {
      const _cpk = String(agreement.pubkey || agreement.partyA?.pubkey || agreement.party_a?.pubkey || agreement.buyerPubkey || '');
      let _line = '';
      try {
        if (!_cpk) { _line = 'Counterparty key unavailable - cannot look up history.'; }
        else {
          const _sc = await (await import('./counterparty_scan')).scanCounterparty(_cpk, String(agreement.network || 'testnet-10'), 15);
          _line = _sc.total > 0
            ? _sc.settled + ' settled\n' + (_sc.deadlocked + _sc.open) + ' unresolved\n' + _sc.total + (_sc.truncated ? '+' : '') + ' escrows total\n\nTraded with ' + _sc.distinctCounterparties + ' distinct part' + (_sc.distinctCounterparties === 1 ? 'y' : 'ies') + '\n\nRead from Kaspa consensus. Unresolved means the trade did not settle - it does not say who walked away.'
            : 'No escrow history found on-chain for this key.';
        }
      } catch (_qe) { console.warn('[Scan] accept-gate lookup failed:', _qe); _line = 'On-chain history unavailable right now.'; }
      Alert.alert('Counterparty history', _line, [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Accept', onPress: () => { handleAcceptFromInbox({ ...agreement, _kvScanned: true }); } },
      ]);
      return;
    }
    ...
    try {
      ...
      if (true) {
        ...
        try {
          ...
          /* LOCAL-TIMEOUT-FILL: device record is authoritative - TownHall snake_case records drop timeoutN. */
        ...
        try {
              { /* REUSE-GUARD: never rebuild a frozen template - a rebuild invalidates the buyer cosign */
        ...
        if (_rgP && _rgB) {
          ...
          setStep(4); Alert.alert('Templates Re-Copied — paste in the AMBER box', 'Your existing frozen templates were copied again (unchanged). Send both to the buyer — they paste this into the AMBER refund-template box on their screen. When they send back their co-signature, paste it in your PURPLE box below. Nothing was re-built.');
          setIsLoading(false); setAcceptingId(null); return;
        }
      }
      console.log('[Neighbor] Seller preparing (freeze, no broadcast)', immediateSendAmount / 1e8, 'KASPA to FROST');
              ...
              const txResult = await sendKaspaViaRest({
                ...
                // so two spends of the same escrow with different outputs would share k
                // and leak the key by subtraction. Nothing signed with it — the closure
                // always returned '' anyway. Use generateNonce() from the canonical
                // steps module (random k per call) for any signing.
                payload: '',
              });
...
killNonces: _kill.nonces.map((n: any) => ({ k: n.k.toString(16), d_tweaked: n.d_tweaked.toString(16), R_hex: n.R_hex })),
      ...
      laUpsert({ agrId, predictedFundingTxId: txResult.predictedTxId }).then(() => laStep(agrId, 'templates_built')).catch(() => {});
          ...
          try {
            ...
            addToFrostList({
              agrId: agrId,
              frostAddr: frostData.address,
              role: canon?.role as any || 'seller', timeoutN: Number(canon?.timeoutN || 0),
              step: 3,
  ...
  const handleEnterDispute = async () => {
    const newStats = {
      ...userStats,
    ...
    <Modal visible={visible} animationType="slide" transparent>
          ...
          <ScrollView style={styles.content} contentContainerStyle={styles.contentContainer} keyboardShouldPersistTaps="handled" onScrollBeginDrag={Keyboard.dismiss}>
            ...
            {step === 1 && !agreementType && (
              <View>
                    ...
                    onPress={async () => {
                      ...
                      try {
                        ...
                        setContract({
                          ...
                          timeoutMinutes: Math.max(1, Math.round(Number(_p.timeoutN || 3000) / DAA_PER_MIN)),
                ...
                <TouchableOpacity
                  onPress={() => { setAgreementType('join'); loadInbox(); loadLocalList(); }}
            ...
            {step === 1 && agreementType === 'join' && (
              <View>
                  ...
                  {manualLookupResult && !manualLookupResult._terminal && (
                    <View style={{ backgroundColor: '#f0fdf4', borderRadius: 8, padding: 10, marginBottom: 8, borderWidth: 1, borderColor: '#86efac' }}>
                      ...
                      <View style={{ marginTop: 8 }}>
                        ...
                        <TextInput
                          style={{ backgroundColor: '#fff', borderWidth: 2, borderColor: manualVerCode.length === 12 ? '#16a34a' : '#fbbf24', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, fontSize: rs.font(18), fontFamily: 'monospace', textAlign: 'center', letterSpacing: 8, color: '#1c1917' }}
                          placeholder="665969B73196"
                          placeholderTextColor="#d6d3d1"
                          value={manualVerCode}
                          onChangeText={(t) => setManualVerCode(t.toUpperCase().slice(0, 12))}
                    ...
                    onPress={async () => {
                      ...
                      try {
                        ...
                        if (match) {
                          ...
                          // RECEIPT-LOOKUP: not an open proposal — check for a terminal inscription (completed trade)
                          try {
                ...
                {false && inboxAgreements.length > 0 && (
      ...
      laUpsert({ agrId: parsed.agrId, role: _rolePaste as any, origin: _rolePaste === 'buyer' ? 'mine' : 'given', buyerPubkey: parsed.buyerPubkey, sellerPubkey: parsed.sellerPubkey, buyerAmountSompi: String(parsed.buyerAmountSompi ?? ''), sellerAmountSompi: String(parsed.sellerAmountSompi ?? ''), frostCounter: parsed.frostCounter, timeoutN: parsed.timeoutN, network: parsed.network, description: parsed.description, verificationCode: parsed.verificationCode, buyerR: parsed.buyerR }).catch(() => {});
                          ...
                          pastedTypeRef.current = (parsed as any).agreementType === "simple" ? "simple" : ((parsed as any).agreementType === "trade" ? "trade" : null); console.log("[Type-Field] pasted agreementType:", pastedTypeRef.current);
                            let _kvScanLine = '';
                            try {
                              const _cpPk = String((_rolePaste === 'buyer' ? parsed.sellerPubkey : parsed.buyerPubkey) || '');
                              if (_cpPk) {
                                const _sc = await (await import('./counterparty_scan')).scanCounterparty(_cpPk, String(parsed.network || 'testnet-10'), 15);
                                _kvScanLine = _sc.total > 0
                                  ? '\n\nOn-chain history\n' + _sc.settled + ' settled, ' + (_sc.deadlocked + _sc.open) + ' unresolved of ' + _sc.total + (_sc.truncated ? '+' : '') + '\nTraded with ' + _sc.distinctCounterparties + ' distinct part' + (_sc.distinctCounterparties === 1 ? 'y' : 'ies')
                                  : '\n\nOn-chain history: none found';
                              }
                            } catch (_qe) { console.warn('[Scan] pre-trade lookup failed:', _qe); _kvScanLine = '\n\nOn-chain history: unavailable'; }
                            Alert.alert("Proposal Found", "Item: " + (parsed.description || "N/A") + "\nAmount: " + (Number(parsed.buyerAmountSompi || 0)/1e8) + " KAS\nCode: " + (parsed.verificationCode || "") + _kvScanLine, [
                              { text: "Cancel", style: "cancel" },
                              { text: "Accept", onPress: () => {
                ...
                {false && [...inboxAgreements].sort((a, b) => {
                    ...
                    {contract.agreementId && (
                      <View style={{ backgroundColor: '#fffbeb', borderRadius: 12, borderWidth: 2, borderColor: '#f59e0b', padding: 16, marginBottom: 16 }}>
                        ...
                        <TouchableOpacity onPress={async () => { 
                          ...
                          /* BODY-IMMUTABLE: a proposal body is generated ONCE. Re-copying must reuse the stored body verbatim - regeneration with a fresh R/counter forks the ceremony under the same agrId. */
                          ...
                          if (_storedRec && _storedRec.proposalBody && _storedRec.proposalBody.startsWith('KV|')) {
                            Clipboard.setStringAsync('KasVillage agreement proposal. Open Neighbor Agreement and paste ALL of this into the box titled "Paste Buyer Proposal" (blue). /* PASTE-INSTRUCTIONS */\n\n' + _storedRec.proposalBody);
  ...
  { label: 'Network', value: contract.frostData?.network || 'testnet-10', detail: '' },
                    ...
                    {role === 'buyer' && contract.multisigAddress && (
                      <View style={{ backgroundColor: '#fffbeb', borderRadius: 12, borderWidth: 2, borderColor: '#f59e0b', padding: 14, marginBottom: 16 }}>
                        <Text style={{ fontSize: rs.font(13), fontWeight: 'bold', color: '#92400e', marginBottom: 4 }}>BUYER STEP 1 — Co-sign Seller's Refund</Text>
                    ...
                    {role === 'buyer' && contract.multisigAddress && (
                      <View style={{ backgroundColor: '#eef2ff', borderRadius: 12, borderWidth: 2, borderColor: '#6366f1', padding: 14, marginBottom: 16 }}>
                        <Text style={{ fontSize: rs.font(13), fontWeight: 'bold', color: '#3730a3', marginBottom: 4 }}>BUYER STEP 2 — Paste Kill Tx from Seller</Text>
...
nonces: _killNonces,
                              ...
                              try {
                                ...
                                if (await SecureStore.getItemAsync('kv_funded_' + _agrId)) { console.log('[Refund] FUND-SENTINEL - already funded, skipping re-inscribe + re-fund'); setIsLoading(false); return; }
              ...
              <TouchableOpacity 
                ...
                onPress={async () => {
                  try {
                    ...
                    const myR = await (async () => { try { const s = await SecureStore.getItemAsync('kv_frost_nonce_' + (contract.agreementId || '')); if (s) return JSON.parse(s).R_hex || ''; } catch {} return ''; })(); await Clipboard.setStringAsync('AGR: ' + (contract.agreementId || '') + '\nR: ' + myR + '\nSIG: ' + (contract.partialReleaseTx || '') + '\nSeller: press Check for Release');
