...
function frame(subject, opts) {
  ...
  rows.length = 0;
  rows.push("K".repeat(W));
  for (let i = 0; i < 11; i++) rows.push("s".repeat(W));
...
const scene = {
  ...
  meta: { id: "cell_workbench4", name: "Pixel Cell Workbench 4", seed: "cw4", players: 1, category: "tool" },
...
fs.writeFileSync("cell_workbench4.json", json);
...
(function () {
  ...
  // comic lettering: heavy weight, cream fill, ink outline (ref burst cards)
  function comicify() {
    try {
      var el = nodes.cap._el, es = nodes.sub._el;
      el.style.fontWeight = "900"; el.style.fontSize = "34px";
      el.style.color = "#f8f0d8"; el.style.letterSpacing = "2px";
      el.style.fontFamily = "Impact, 'Arial Black', sans-serif";
      el.style.textShadow = "3px 3px 0 #241c12, -2px -2px 0 #241c12, 2px -2px 0 #241c12, -2px 2px 0 #241c12";
      es.style.fontWeight = "700"; es.style.color = "#f4e4c1";
      es.style.textShadow = "2px 2px 0 #241c12";
    } catch (err) {}
  }
  setTimeout(comicify, 400);
...
fs.writeFileSync("cell_workbench4.html",
  engine.replace("</script>", "\ntry { loadScene(" + JSON.stringify(json) +
    "); } catch (e) { console.error('wb4 boot: ' + (e && e.message)); }\n</script>") + switcher);

console.log("OK cell_workbench4.json (" + (json.length / 1024).toFixed(1) + " KB, " +
  total + " runs, " + CELLS.length + " cells)");
console.log("OK cell_workbench4.html — 1-9,0,q,w to switch; +/- size");
