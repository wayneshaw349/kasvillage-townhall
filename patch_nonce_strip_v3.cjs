// patch_nonce_strip_v3.cjs — remove the dead nonce generator call from the accept path.
// v2 aborted because the replacement COMMENT contained the identifier, tripping the
// post-condition. v3 keeps the identifier out of the inserted text.
//
// WHY: that generator derives k = blake2b(d_tweaked || message) where
...
//
// Run: node patch_nonce_strip_v3.cjs
const fs = require('fs');

const F = 'NeighborAgreement.tsx';
const ID = 'generate' + 'FrostNonce';   // split so this script's own text can't confuse a grep
...
const CALL = "                payload: await (async () => { try { const nonce = " + ID + "({";
const IMP  = "  " + ID + ",";   // no newline — file is CRLF
requireCount('payload closure', CALL, 1);
requireCount('import line', IMP, 1);
requireCount('total references', ID, 2);
...
const before = s;
s = s.replace(new RegExp(" {2}" + ID + ",\\r?\\n"), '');
if (s === before) { console.error('ABORT: import removal was a NO-OP'); process.exit(1); }

// ---- post-conditions ----
const left = occurrences(s, ID);
if (left !== 0) { console.error('ABORT: ' + left + ' references remain'); process.exit(1); }
