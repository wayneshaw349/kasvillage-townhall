...
export const AppNavigator: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const privateKey: string = (await SecureStore.getItemAsync('kv_private_key'))
          || (await SecureStore.getItemAsync(STORE_KEYS.PRIVATE_KEY)) || '';
        const privKeyEnc: string = (await SecureStore.getItemAsync('kv_l1_privkey_enc')) || '';
        const kaspaAddrBoot: string = (await SecureStore.getItemAsync('kv_kaspa_address'))
          || (await SecureStore.getItemAsync('kaspa_address')) || '';
        const aptAlias: string = (await SecureStore.getItemAsync('kv_apt_alias'))
          || (await SecureStore.getItemAsync(STORE_KEYS.APT_ALIAS)) || '';
        const kvVerified: string = (await SecureStore.getItemAsync('kv_verified')) || '';
        const hasWallet = !!((privateKey || privKeyEnc) && kaspaAddrBoot);
        console.log('[AppNav] boot keys — priv:', !!privateKey, 'privEnc:', !!privKeyEnc,
          'addr:', !!kaspaAddrBoot, 'alias:', !!aptAlias, 'verified:', kvVerified || 'none');
        ...
        const publicKey: string = (await SecureStore.getItemAsync('kv_public_key'))
          || (await SecureStore.getItemAsync(STORE_KEYS.PUBLIC_KEY))
          || (await SecureStore.getItemAsync('kaspa_pubkey')) || ''; console.log('[AppNav] Your PubKey:', publicKey);
        try {
          const _C='qpzry9x8gf2tvdw0s3jn54khce6mua7l';
          const _poly=(v: number[])=>{let c=1n;for(const d of v){const c0=c>>35n;c=((c&0x07fffffffffn)<<5n)^BigInt(d);if(c0&0x01n)c^=0x98f2bc8e61n;if(c0&0x02n)c^=0x79b76d99e2n;if(c0&0x04n)c^=0xf33e5fb3c4n;if(c0&0x08n)c^=0xae2eabe2a8n;if(c0&0x10n)c^=0x1e4f43e470n;}return c^1n;};
          const _c85=(p: number[])=>{const r=[];let b=0,q=0;for(const c of p){b=(b<<8)|c;q+=8;while(q>=5){q-=5;r.push((b>>q)&31);b&=(1<<q)-1;}}if(q>0)r.push((b<<(5-q))&31);return r;};
  ...
  const refreshBalance = useCallback(async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        console.log('[Refresh]', kaspaAddress.slice(0, 20), '->', data.balance);
