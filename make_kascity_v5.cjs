// make_kascity_v3.cjs — KasCity v3. Emits kascity_v5.json + showcase_kascity5.html.
//
...
for (let s = 1; s <= PLAYERS; s++) {
  bootActions.push({ action: "setSeatStat", args: [s, "cash", 1500] });
...
const scene = {
  kind: "kv_game_v1",
  engine: "scene",
  meta: { id: "kascity_v5", name: "KasCity", seed: "kc5", players: PLAYERS, category: "board" },
  debug: false,
...
fs.writeFileSync("kascity_v5.json", json);
console.log("OK kascity_v5.json (" + (json.length / 1024).toFixed(1) + " KB, BT " +
  (JSON.stringify(bt).length / 1024).toFixed(1) + " KB, nodes " + nodes.length + ")");

const engine = fs.readFileSync("scene_engine.html", "utf8");
const inject = [
  "", "// ---- injected kascity v5 showcase ----",
  "try { loadScene(" + JSON.stringify(json) + "); }",
  "catch (e) { console.error('kascity5 boot: ' + (e && e.message)); }", ""
].join("\n");
fs.writeFileSync("showcase_kascity5.html",
  engine.replace("</script>", inject + "\n</script>"));
console.log("OK showcase_kascity5.html");
