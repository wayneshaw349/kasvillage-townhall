// make_kascity_v3.cjs — KasCity v3. Emits kascity_v9.json + showcase_kascity9.html.
//
...
const materials = {
  ...
  scrim: { color: "#0b0f0d" },
  cel_deed: { color: "#8fd6a8" }, cel_pipes: { color: "#9cc4e8" }, cel_storm: { color: "#b9bcd6" },
  cel_walks: { color: "#e8cf9c" }, cel_court: { color: "#e8a3a3" }, cel_bust: { color: "#b0b0b0" }, roof: { color: "#a03d2e" }, monu: { color: "#b8a878" },
...
// ---------------------------------------------------------------------------
const CELL = [
  { id: "deed",  cap: "DEED SECURED",   sub: "the block is yours",        col: "#2f7d4f", icon: "scroll" },
  { id: "pipes", cap: "PIPES BURST",    sub: "emergency plumbing",        col: "#2f5f8e", icon: "pipe" },
  { id: "storm", cap: "STORM DAMAGE",   sub: "the roof gave way",         col: "#4a4a6a", icon: "cloud" },
  { id: "walks", cap: "TENANT WALKS",   sub: "lease is up",               col: "#8a6a2f", icon: "door" },
  { id: "court", cap: "COURT SUMMONS",  sub: "you are being sued",        col: "#7a2f2f", icon: "gavel" },
  { id: "bust",  cap: "BANKRUPT",       sub: "the city takes it all",     col: "#3a3a3a", icon: "skull" }
];

// icon silhouettes: each is a list of [mesh, x, y, z, rotY]
const ICONS = {
  scroll: [["ic_bar", 0, 0.9, 0, 0], ["ic_bar", 0, -0.9, 0, 0], ["ic_sheet", 0, 0, -0.05, 0]],
  pipe:   [["ic_tube", -0.7, 0.4, 0, 0], ["ic_tube", 0.7, -0.2, 0, 0], ["ic_drop", 0.1, -0.9, 0.1, 45]],
  cloud:  [["ic_puff", -0.7, 0.5, 0, 0], ["ic_puff", 0.2, 0.75, 0, 0], ["ic_puff", 0.9, 0.45, 0, 0],
           ["ic_rain", -0.4, -0.6, 0, 20], ["ic_rain", 0.4, -0.7, 0, 20]],
  door:   [["ic_sheet", -0.3, 0, 0, 0], ["ic_knob", 0.25, 0, 0.08, 0], ["ic_rain", 1.1, -0.2, 0, 70]],
  gavel:  [["ic_tube", 0.2, 0.5, 0, 55], ["ic_head", -0.5, 1.0, 0, 55], ["ic_bar", 0, -0.9, 0, 0]],
  skull:  [["ic_head", 0, 0.35, 0, 0], ["ic_knob", -0.28, 0.45, 0.3, 0], ["ic_knob", 0.28, 0.45, 0.3, 0],
           ["ic_bar", 0, -0.55, 0.2, 0]]
};
...
CELL.forEach(function (c, ci) {
  ...
  const kids = [
    ...
    { id: "cel_bg_" + c.id,    mesh: "cell_bg",    material: "cel_" + c.id, transform: { pos: [0, 0.12, 0] } },
    ...
    { id: "cel_sp1_" + c.id,   mesh: "speed",      material: "ink", transform: { pos: [-5.6, 0.22, 2.4], rot: [0, 24, 0] } },
    { id: "cel_sp2_" + c.id,   mesh: "speed",      material: "ink", transform: { pos: [5.6, 0.22, -2.3], rot: [0, -24, 0] } }
  ];
  (ICONS[c.icon] || []).forEach(function (p, k) {
    ...
    kids.push({ id: "cel_ic" + k + "_" + c.id, mesh: p[0], material: "ink",
      transform: { pos: [p[1] * 1.35, 0.34, -p[2] * 1.35], rot: [0, p[4], 0] } });
  });
...
const scene = {
  kind: "kv_game_v1",
  engine: "scene",
  meta: { id: "kascity_v9", name: "KasCity", seed: "kc9", players: PLAYERS, category: "board" },
  debug: false,
  ...
  resources: {
    meshes: {
      ...
      cell_bg: { type: "box", size: [17.4, 0.08, 8.8] },
      cell_bar: { type: "box", size: [17.4, 0.1, 0.42] },
      cell_side: { type: "box", size: [0.42, 0.1, 8.8] },
      speed: { type: "box", size: [4.2, 0.08, 0.16] },
      ic_bar: { type: "box", size: [3.1, 0.16, 0.4] },
      ic_sheet: { type: "box", size: [2.6, 0.16, 2.3] },
      ic_tube: { type: "box", size: [2.0, 0.16, 0.56] },
      ic_drop: { type: "box", size: [0.46, 0.16, 0.46] },
      ic_puff: { type: "box", size: [2.0, 0.16, 1.1] },
      ic_rain: { type: "box", size: [0.22, 0.16, 1.2] },
      ic_knob: { type: "box", size: [0.4, 0.16, 0.4] },
      ic_head: { type: "box", size: [1.7, 0.16, 1.35] },
...
const checks = [
  ...
  ["icons above panel", json.indexOf('0.34') >= 0 && json.indexOf('"cell_scrim"') >= 0]
];
let bad = 0;
...
fs.writeFileSync("kascity_v9.json", json);
console.log("OK kascity_v9.json (" + (json.length / 1024).toFixed(1) + " KB, BT " +
  (JSON.stringify(bt).length / 1024).toFixed(1) + " KB, nodes " + nodes.length + ")");

const engine = fs.readFileSync("scene_engine.html", "utf8");
const inject = [
  "", "// ---- injected kascity v9 showcase ----",
  "try { loadScene(" + JSON.stringify(json) + "); }",
  "catch (e) { console.error('kascity9 boot: ' + (e && e.message)); }", ""
].join("\n");
fs.writeFileSync("showcase_kascity9.html",
  engine.replace("</script>", inject + "\n</script>"));
console.log("OK showcase_kascity9.html");
