...
The block's fields (see Appendix A): `ambient` is a COLOR STRING
(`"#404850"` — a number crashes the color parser), `ambientIntensity` is the
numeric strength (default 0.35), and the ambient value doubles as a darkness
overlay at draw time — a dark or opaque ambient dims or blacks the whole
frame. There is no `sun` key. A bright board game usually reads best with no
`lighting` block at all.
...
```json
"alarms": [
  { "after": 0.1, "do": { "action": "setState", "args": ["seat", 1] } },
  { "after": 0.1, "do": { "action": "setState", "args": ["go", -1] } },
  { "after": 0.1, "do": { "action": "setState", "args": ["phase", 0] } },
  { "after": 0.1, "do": { "action": "shuffleDeck", "args": ["fate"] } },
  { "after": 0.1, "do": { "action": "setState", "args": ["ready", 1] } }
]
```

One `{ after, repeat, do }` entry per action — there is no `at` and no
`actions` array (those keys are silently ignored). Entries fire in
registration order within a tick, so `ready` in the **last** entry still
lands last, and the director's tree opens with it:
...
## Appendix A: Silent Keys — The Debugging Method (KasCity v3 session, 2026-08-20)

This appendix records how two silent-key bugs and one crash were found and
fixed, as a repeatable method. It is the worked companion to Part 9's
"Silence is the engine's worst habit".

## A. What happened (three bugs, one evening)

1. **Lighting crash → black screen.** The descriptor declared
   `lighting: { ambient: 0.55, sun: {...} }`. The engine's `hexRGB(cfg.ambient)`
   called `.replace` on a number and threw *inside* loadScene — no "scene
   rejected", just a dead frame and touch controls. Fixing the type
   (`ambient` is a hex STRING) then revealed bug 2: the engine's darkness
   overlay (`cfg.ambient` used as a canvas fill at draw time) painted over the
   whole frame. Removing the lighting block entirely restored the render.
2. **Boot alarm silently ignored.** The descriptor used
   `alarms: [{ id, at, actions: [...] }]` — the shape shown in this manual's
   own Part 7 example. The engine's real schema (v6.5) is per-entry
   `{ after, repeat, do: { action, to, args, amount } }`, ONE action per alarm.
   Wrong keys → no error, no warning, no seeding: every flag read 0 and the
   game never offered a roll.
3. **Manual drift.** Part 7's boot example and Part 4's lighting mention were
   both wrong or incomplete against the shipped engine. The manual is a
   document; the engine source is the schema.

## B. The method (step by step)

When a scene renders wrong, is black, or "does nothing":

1. **Read the console first.** A thrown error names the function
   (`(h || "#ffffff").replace is not a function` → the color parser → a color
   field got a non-string). No error + controls visible = the scene loaded and
   the fault is draw- or logic-side.
2. **Confirm the bytes you think you're running.** Grep the served HTML for a
   string only the new build contains. Stale cache and stale emit are the two
   cheapest wrong turns.
3. **Probe live state in the browser console, one JSON line at a time:**
   - `{mode, cam, camPos, nodeCount, playing}` — camera and loop
   - `{geo, worldPos}` on a known node — geometry built and placed
   - drawable count (`_geo && !hidden`) — culling/visibility
   - canvas `{w, h}` and `BAKE.done` — surface and bake
   - `world.flags` — if every flag you seeded is `undefined`, your boot never
     ran: suspect a silent key.
4. **Bisect by descriptor subtraction, not by guessing:** re-fetch the
   pristine JSON, delete one block, `loadScene` it again:
   `fetch('x.json').then(r=>r.text()).then(t=>{var s=JSON.parse(t); delete s.lighting; loadScene(JSON.stringify(s));})`
   (The live `scene` object gains circular `_parent` refs — never re-stringify
   it; always go back to the file.)
5. **When a block is implicated, read the engine's consumer, not the manual.**
   Grep the engine for the key (`ambient`, `alarmRegister`) and read the ten
   lines around it. The destructuring in the source IS the schema:
   `a.def.after || 1` and `a.def.do` told the whole alarm story in two lines.
6. **Fix the generator, never the emitted JSON**, and add a self-check to the
   generator that greps its own output for the fixed shape, so the regression
   can't come back silently.

## C. Corrected schemas (authoritative as of engine build ~302 KB)

### Alarms — per node, one action each
```json
"alarms": [
  { "after": 0.1, "do": { "action": "setState", "args": ["seat", 1] } },
  { "after": 0.1, "do": { "action": "shuffleDeck", "args": ["fate"] } },
  { "after": 0.1, "do": { "action": "setState", "args": ["ready", 1] } },
  { "after": 10, "repeat": true, "do": { "action": "spawn", "args": ["wave", 0, 0, -8] } }
]
```
- `after` — seconds until firing (repeats on that period with `repeat: true`).
- `do` — ONE action, engine-standard shape; `to` names a target (default self).
- Entries fire in registration order within a tick, so "set `ready` last"
  still works as the boot pattern — put it in the final entry.
- There is no `at`, no `actions` array, no alarm `id`. Those keys are ignored
  without complaint.

### Lighting
- `ambient` is a COLOR STRING (hex like `"#404850"`, or rgba). Numbers crash
  the color parser.
- `ambientIntensity` is the numeric knob (default 0.35).
- The ambient value is also used as a darkness overlay at draw time: a dark
  or opaque ambient dims or blacks the frame. A bright board game generally
  reads better with NO lighting block at all (baking skipped, flat colors).
- There is no `sun` key.

## D. What to harden in the engine (proposals from this session)

1. Wrap `hexRGB` inputs: non-string → warn `[kv-schema] lighting.ambient must
   be a color string` and fall back to default, instead of throwing mid-load.
2. Extend the `[kv-schema]` load-time warnings to cover the two shapes that
   burned this session: `alarms[].at` / `alarms[].actions` ("did you mean
   after/do?") and numeric `lighting.ambient`.
3. Consider a debug-mode strict flag: unknown keys reject instead of warn, for
   authoring only.

One sentence to remember: **if a rule is not firing, the first question is
never "is my logic wrong" — it is "is the engine reading this key at all",
and the engine's own source answers it faster than the manual.**

---

*Manual current as of engine build 302 KB, tests at 156 green, August 2026. Rev 2: corrected alarm schema (after/do, not at/actions), documented lighting schema, added Appendix A (silent-key debugging method).*
