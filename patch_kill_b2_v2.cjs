// patch_kill_b2_v2.cjs — kill tx: paste 4 + buyer enforcement.
// v2: fixes a bad post-condition in v1 (it looked for "kv_kill_' + contract.agreementId"
//     but the paste-4 UI writes "kv_kill_' + (contract.agreementId || '')"). v1 aborted
//     before writing, so this applies cleanly to an untouched file.
...
// ============ post-conditions ============
const nGate = occurrences(s, 'Kill-Gate');
if (nGate < 6) { console.error('ABORT: expected >=6 Kill-Gate refs, saw ' + nGate); process.exit(1); }
const nAgg = occurrences(s, 'buyerAggregate({');
if (nAgg !== 3) { console.error('ABORT: expected 3 buyerAggregate calls, saw ' + nAgg); process.exit(1); }
const nKill = occurrences(s, "kv_kill_");   // paste4 store + poll read + recovery read
if (nKill < 3) { console.error('ABORT: expected >=3 kv_kill_ refs, saw ' + nKill); process.exit(1); }
if (occurrences(s, '[Refund] Signed refund STORED') !== 1) { console.error('ABORT: 2d block damaged'); process.exit(1); }
if (occurrences(s, 'buildKillTx({') !== 1) { console.error('ABORT: B1 build call lost'); process.exit(1); }

fs.writeFileSync(F, s);
console.log('WROTE ' + F);
console.log('Kill-Gate refs: ' + nGate + ' | buyerAggregate calls: ' + nAgg + ' | kv_kill_ refs: ' + nKill);
