...
// Replace the WHOLE line containing a unique needle. Whitespace-proof: the exact
// line text does not have to be guessed.
function subLine(needle, newLines, tag) {
  const n = s.split(needle).length - 1;
  if (n !== 1) { console.error(`SKIP [${tag}] needle count = ${n}, expected 1`); return; }
  ...
  s = s.slice(0, ls) + newLines.map(l => (l ? ind + l : '')).join(NL) + s.slice(le);
  applied++; console.log(`APPLIED [${tag}]`);
}
...
subLine("JSON.stringify({ k: result.nonce.k.toString(16)",
  ['// v2: one k per input, array order == template u[] order.',
...
const post = [
  ...
  // (dropped: the storage write legitimately contains this exact string)
  ['result.nonces.map', 1],
  ['_cNonces.map', 1],
