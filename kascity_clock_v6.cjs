// make_kascity_v3.cjs — KasCity v3. Emits kascity_v6.json + showcase_kascity6.html.
//
...
nodes.push({
  id: "hud", type: "CanvasLayer", children: [
    ...
    { id: "s1", type: "Label", anchor: "topLeft", pos: [10, 10], size: 12, text: "P1  1500" },
    { id: "s2", type: "Label", anchor: "topLeft", pos: [10, 28], size: 12, text: "P2  1500" },
    { id: "s3", type: "Label", anchor: "topLeft", pos: [10, 46], size: 12, text: "P3  1500" },
    { id: "s4", type: "Label", anchor: "topLeft", pos: [10, 64], size: 12, text: "P4  1500" },
    { id: "tile_label", type: "Label", anchor: "bottomCenter", pos: [0, -46], size: 14, text: "" },
    { id: "clock", type: "Label", anchor: "topRight", pos: [-10, 10], size: 14, text: "10:00" },
...
// Countdown readout, coarse buckets so setText is not called every frame.
const MARKS = [600, 540, 480, 420, 360, 300, 240, 180, 120, 90, 60, 30, 20, 10, 5];
for (const m of MARKS) {
  const mm = Math.floor(m / 60), ss = m % 60;
  ROOT.push(seq(
    cond("world.flags.over == 0 && world.flags.left <= " + m + " && world.flags.mark > " + m),
    act("setState", ["mark", m]),
    actTo("setText", "clock", [mm + ":" + (ss < 10 ? "0" + ss : ss)]),
    (m <= 30 ? act("playSound", ["tick"]) : cond("1 == 1"))
  ));
}
...
const flagSeed = { phase: 0, asked: 0, pos: 0, sum: 0, moved: 1, seat: 1, turn: 0,
  ...
  t0: 0, left: 600, mark: 9999, over: 0,
...
const scene = {
  kind: "kv_game_v1",
  engine: "scene",
  meta: { id: "kascity_v6", name: "KasCity", seed: "kc6", players: PLAYERS, category: "board" },
  debug: false,
...
const checks = [
  ...
  ["clock label present", json.indexOf('"clock"') >= 0 && json.indexOf('"final"') >= 0]
];
let bad = 0;
...
fs.writeFileSync("kascity_v6.json", json);
console.log("OK kascity_v6.json (" + (json.length / 1024).toFixed(1) + " KB, BT " +
  (JSON.stringify(bt).length / 1024).toFixed(1) + " KB, nodes " + nodes.length + ")");

const engine = fs.readFileSync("scene_engine.html", "utf8");
const inject = [
  "", "// ---- injected kascity v6 showcase ----",
  "try { loadScene(" + JSON.stringify(json) + "); }",
  "catch (e) { console.error('kascity6 boot: ' + (e && e.message)); }", ""
].join("\n");
fs.writeFileSync("showcase_kascity6.html",
  engine.replace("</script>", inject + "\n</script>"));
console.log("OK showcase_kascity6.html");
