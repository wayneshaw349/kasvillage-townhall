// make_kascity_v3.cjs — KasCity v3. Emits kascity_v4b.json + showcase_kascity4b.html.
//
...
const scene = {
  kind: "kv_game_v1",
  engine: "scene",
  meta: { id: "kascity_v4", name: "KasCity", seed: "kc4", players: PLAYERS, category: "board" },
  debug: false,
...
fs.writeFileSync("kascity_v4b.json", json);
console.log("OK kascity_v4b.json (" + (json.length / 1024).toFixed(1) + " KB, BT " +
  (JSON.stringify(bt).length / 1024).toFixed(1) + " KB, nodes " + nodes.length + ")");

const engine = fs.readFileSync("scene_engine.html", "utf8");
const inject = [
  "", "// ---- injected kascity v4b showcase ----",
  "try { loadScene(" + JSON.stringify(json) + "); }",
  "catch (e) { console.error('kascity4b boot: ' + (e && e.message)); }", ""
].join("\n");
fs.writeFileSync("showcase_kascity4b.html",
  engine.replace("</script>", inject + "\n</script>"));
console.log("OK showcase_kascity4b.html");
