...
const moveRe = /[ \t]*async function move\(seat, action, arg\)[ \t]*\{[\s\S]*?\n[ \t]*\}[ \t]*\n[ \t]*window\.KV_MOVE[ \t]*=[ \t]*move;/;
...
const resRe = /[ \t]*window\.KV_RESULT=\{[ \t]*\n[ \t]*kind:"kascity\.result\.v1"[\s\S]*?seats:rows[ \t]*\n[ \t]*\};/;
...
const hasBell = bellRe.test(html);
if (hasBell) html = html.replace(bellRe, [
  ...
  // no log line in this build — attach the self-check to the result assembly instead
  const rt = 'window.KV_RESULT.selfVerified';
  if (html.indexOf(rt) < 0) {
    const marker = '    var ov=el("div","position:fixed;inset:0;z-index:70;';
    if (html.indexOf(marker) < 0) die('cannot anchor the self-check');
    html = html.replace(marker, [
      ...
      '        var good = (c === window.KV_RESULT.moveRoot);',
      '        window.KV_RESULT.selfVerified = good;',
      '        if(window.KV_LOG) window.KV_LOG(good ? "record self-verifies \\u2713" : "RECORD FAILED SELF-VERIFY", good ? "#9cd87c" : "#ff6a4a");',
      '        if(!good && window.KV_SHOUT) window.KV_SHOUT("PROOF FAILED","move root does not match the log","#ff6a4a",true);',
      '      }catch(e){}',
      '    })();',
      marker
