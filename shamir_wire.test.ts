...
import {
  encodeShare, decodeShare, splitWithVerify, recoverFromWires,
} from './shamir_wire';
...
// ---------------------------------------------------------------------------
{
  ...
  for (let t = 0; t < 1000; t++) {
    const secret = rng(32);
    const shares = split(secret, 2, 4, (t % 7) + 1, rng);
    for (const sh of shares) {
      const back = decodeShare(encodeShare(sh));
      if (back.index !== sh.index || back.gen !== sh.gen ||
          back.threshold !== sh.threshold || back.total !== sh.total ||
          !eq(back.data, sh.data)) { ok = false; break; }
    }
    if (!ok) break;
  }
...
// ---------------------------------------------------------------------------
{
  ...
  for (let t = 0; t < 500; t++) {
    const secret = rng(32);
    const { wires } = splitWithVerify(secret, 2, 4, 1, rng);
    if (wires.length !== 4) { ok = false; break; }
    // any 2 of the 4 wires recover
    const rec = recoverFromWires([wires[1], wires[3]]);
...
// ---------------------------------------------------------------------------
{
  ...
  let threw = false;
  try { decodeShare(corrupted); } catch { threw = true; }
...
{
  const secret = rng(32);
  const g1 = splitWithVerify(secret, 2, 4, 1, rng).wires;
  const g2 = splitWithVerify(secret, 2, 4, 2, rng).wires;
  let threw = false;
  try { recoverFromWires([g1[0], g2[1]]); } catch { threw = true; }
...
{
  const secret = rng(32);
  const wires = splitWithVerify(secret, 3, 5, 1, rng).wires;
  let threw = false;
  try { recoverFromWires([wires[0], wires[1]]); } catch { threw = true; }
...
// ---------------------------------------------------------------------------
{
  ...
  for (let t = 0; t < 200; t++) {
    try { splitWithVerify(rng(32), 2, 4, 1, rng); }
    catch { threwUnexpectedly = true; break; }
  }
...
// ---------------------------------------------------------------------------
{
  const { wires } = splitWithVerify(rng(32), 2, 4, 1, rng);
