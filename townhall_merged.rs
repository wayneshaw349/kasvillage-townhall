...
impl ArweaveStateReader {
    ...
    pub async fn get_user_stats(&self, pubkey: &str) -> Result<UserCompletionStats, String> {
            ...
            .and_then(|v| v.as_str())
        {
            ...
            let data = self.http_client.get(&url).send().await
                .map_err(|e| format!("Fetch failed: {}", e))?;
            let mut stats: UserCompletionStats = data.json().await
                .map_err(|e| format!("Parse stats failed: {}", e))?;
            stats.pubkey = pubkey.to_string();
            return Ok(stats);
        }
        Ok(UserCompletionStats::new().with_pubkey(pubkey))
    }
...
pub fn generate_user_proof(stats: &UserStatsL1, traits: &CitadelTraits) -> VerificationProof {
    ...
    hasher.update(&[traits.count()]);
    hasher.update(&current_timestamp().to_le_bytes());
    VerificationProof {
        proof_type: "user".into(),
        subject_id: stats.pubkey_hash.clone(),
        verified,
        proof_bytes: hex::encode(hasher.finalize()),
        public_inputs: vec![stats.xp.to_string(), format!("{:.2}", stats.p_complete()), traits.count().to_string()],
        timestamp: current_timestamp(),
    }
}
...
async fn query_dapp(
    ...
    let risk_assessment = verification.as_ref().map(|v| {
        ...
        let risk_level = if risk_score < 0.2 { CodeRiskLevel::Safe }
            ...
            else if risk_score < 0.9 { CodeRiskLevel::High }
        ...
        let recommendation = match risk_level {
            ...
            CodeRiskLevel::High => "🚨 High risk - not recommended",
...
async fn query_dapp_risk(
    ...
    if let Some(dapp_id) = &body.dapp_id {
        ...
        if let Some(v) = verification {
            ...
            return HttpResponse::Ok().json(DAppRiskAssessment {
                dapp_id: v.dapp_id,
                risk_level: if v.code_scan.passed { CodeRiskLevel::Safe } else { CodeRiskLevel::High },
                risk_score,
...
async fn get_host_nodes(
    ...
    let resp = match state.arweave.http_client
        .post(ARWEAVE_GRAPHQL)
        .json(&serde_json::json!({ "query": query }))
        .send().await
    {
    ...
    for edge in &edges {
        ...
        let url = format!("{}/{}", ARWEAVE_GATEWAY, tx_id);
        let payload: serde_json::Value = match state.arweave.http_client
            .get(&url)
            .send().await
