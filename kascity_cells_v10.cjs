// make_kascity_v3.cjs — KasCity v3. Emits kascity_v10.json + showcase_kascity10.html.
//
...
// ---------------------------------------------------------------------------
// [COMIC CELLS] Full-screen event panels, built from primitives — the engine
// has no image support by design, so "drawn art" means composed boxes. Each
// cell is a hidden group; an event shows it and a per-cell alarm hides it.
// Six cells + the winner card. Icons read as silhouettes at PS1 fidelity.
// ---------------------------------------------------------------------------
const CELL = [
  { id: "deed",  cap: "DEED SECURED",   sub: "the block is yours",    icon: "scroll" },
  { id: "pipes", cap: "PIPES BURST",    sub: "emergency plumbing",    icon: "pipe" },
  { id: "storm", cap: "STORM DAMAGE",   sub: "the roof gave way",     icon: "cloud" },
  { id: "walks", cap: "TENANT WALKS",   sub: "lease is up",           icon: "door" },
  { id: "court", cap: "COURT SUMMONS",  sub: "you are being sued",    icon: "gavel" },
  { id: "bust",  cap: "BANKRUPT",       sub: "the city takes it all", icon: "skull" }
];

// Each cell is a little staged scene: a standing FIGURE plus props, drawn in
// panel space (x across, `up` toward the top of the screen). Layer lifts the
// piece so overlapping shapes still read as separate silhouettes.
const ICONS = {
  // buyer plants a flag on their new block
  scroll: [
    ["fig_body", -1.8, -0.2, 1, 0], ["fig_head", -1.8, 1.15, 1, 0],
    ["fig_arm", -0.9, 0.55, 2, -35],
    ["ic_post", 0.6, 0.1, 1, 0], ["ic_flag", 1.35, 0.95, 1, 0],
    ["ic_house", 3.0, -0.15, 0, 0], ["ic_roof", 3.0, 0.85, 0, 0]
  ],
  // plumber under a bursting pipe
  pipe: [
    ["ic_tube", 0.4, 1.5, 0, 0], ["ic_tube", 2.1, 1.5, 0, 0],
    ["ic_rain", 1.3, 0.6, 1, 0], ["ic_rain", 1.6, -0.1, 1, 12], ["ic_drop", 1.35, -0.8, 1, 0],
    ["fig_body", -1.6, -0.3, 1, 0], ["fig_head", -1.6, 1.05, 1, 0],
    ["fig_arm", -0.7, 0.7, 2, -55]
  ],
  // figure hunched under cloud + rain, house behind
  cloud: [
    ["ic_puff", -1.4, 2.0, 0, 0], ["ic_puff", 0.2, 2.3, 0, 0], ["ic_puff", 1.7, 2.0, 0, 0],
    ["ic_rain", -1.5, 0.9, 1, 15], ["ic_rain", -0.3, 0.5, 1, 15], ["ic_rain", 0.9, 0.9, 1, 15],
    ["ic_rain", 2.0, 0.4, 1, 15],
    ["ic_house", 2.9, -0.4, 0, 0], ["ic_roof", 2.9, 0.6, 0, 0],
    ["fig_body", -2.2, -0.5, 2, 0], ["fig_head", -2.2, 0.85, 2, 0]
  ],
  // tenant walking out of a doorway, suitcase in hand
  door: [
    ["ic_frame", -2.2, 0.15, 0, 0], ["ic_sheet", -2.2, 0.1, 1, 0], ["ic_knob", -1.5, 0.05, 2, 0],
    ["fig_body", 0.9, -0.25, 1, 0], ["fig_head", 0.9, 1.1, 1, 0],
    ["fig_leg", 1.5, -1.15, 1, 22], ["fig_leg", 0.4, -1.15, 1, -18],
    ["ic_case", 2.1, -0.35, 2, 0]
  ],
  // judge behind a bench, gavel raised
  gavel: [
    ["ic_bench", 0.2, -0.9, 2, 0],
    ["fig_body", 0.2, 0.1, 1, 0], ["fig_head", 0.2, 1.45, 1, 0],
    ["fig_arm", 1.2, 1.0, 1, -45],
    ["ic_handle", 2.1, 1.75, 1, 40], ["ic_mallet", 2.85, 2.25, 1, 40],
    ["ic_bar", -2.6, 0.5, 0, 0]
  ],
  // figure slumped, empty pockets, house behind with a cross
  skull: [
    ["ic_house", 2.6, -0.3, 0, 0], ["ic_roof", 2.6, 0.7, 0, 0],
    ["ic_bar", 2.6, 0.2, 1, 38], ["ic_bar", 2.6, 0.2, 1, -38],
    ["fig_body", -1.4, -0.45, 1, 0], ["fig_head", -1.4, 0.8, 1, 12],
    ["fig_arm", -2.3, -0.1, 2, 35], ["fig_arm", -0.5, -0.1, 2, -35]
  ]
};

CELL.forEach(function (c, ci) {
  // Overhead camera looks straight down, so a "comic panel" is a stack of flat
  // planes at increasing Y: scrim (dims the board), panel, border bars, then
  // the ink icon on top. No rotation — the board plane IS the picture plane.
  const kids = [
    { id: "cel_scrim_" + c.id, mesh: "cell_scrim", material: "scrim", transform: { pos: [0, 0.00, 0] } },
    { id: "cel_bg_" + c.id,    mesh: "cell_bg",    material: "paper", transform: { pos: [0, 0.12, 0] } },
    { id: "cel_sky_" + c.id,   mesh: "cell_sky",   material: "sky_" + c.id, transform: { pos: [0, 0.16, -2.1] } },
    { id: "cel_gnd_" + c.id,   mesh: "cell_gnd",   material: "gnd_" + c.id, transform: { pos: [0, 0.16, 3.0] } },
    { id: "cel_fr1_" + c.id,   mesh: "cell_bar",   material: "ink", transform: { pos: [0, 0.20, 4.3] } },
    { id: "cel_fr2_" + c.id,   mesh: "cell_bar",   material: "ink", transform: { pos: [0, 0.20, -4.3] } },
    { id: "cel_fr3_" + c.id,   mesh: "cell_side",  material: "ink", transform: { pos: [-8.6, 0.20, 0] } },
    { id: "cel_fr4_" + c.id,   mesh: "cell_side",  material: "ink", transform: { pos: [8.6, 0.20, 0] } },
    { id: "cel_sp1_" + c.id,   mesh: "speed", material: "ink", transform: { pos: [-7.2, 0.30, -3.1], rot: [0, 28, 0] } },
    { id: "cel_sp2_" + c.id,   mesh: "speed", material: "ink", transform: { pos: [7.2, 0.30, -3.4], rot: [0, -28, 0] } },
    { id: "cel_sp3_" + c.id,   mesh: "speed", material: "ink", transform: { pos: [-7.6, 0.30, 3.4], rot: [0, -20, 0] } }
  ];
  (ICONS[c.icon] || []).forEach(function (p, k) {
    // icon coords are authored in panel space: x across, y (2nd) becomes -z so
    // "up" in the drawing reads as "up" on screen from an overhead camera.
    kids.push({ id: "cel_ic" + k + "_" + c.id, mesh: p[0], material: "ink",
      transform: { pos: [p[1] * 1.5, 0.24 + p[3] * 0.06, -p[2] * 1.5], rot: [0, p[4], 0] } });
  });
  nodes.push({ id: "cell_" + c.id, hidden: true, tags: ["cell"],
    transform: { pos: [0, 1.6, 0] }, children: kids });
});
...
nodes.push({
  id: "hud", type: "CanvasLayer", children: [
    ...
    { id: "cap", type: "Label", anchor: "topCenter", pos: [0, 150], size: 20, text: "", visible: false },
    { id: "capsub", type: "Label", anchor: "topCenter", pos: [0, 176], size: 13, text: "", visible: false }
  ]
});
...
// Raise comic cell `id` (index into CELL) with its caption.
const popCell = (name) => {
  const ci = CELL.findIndex(function (c) { return c.id === name; });
  const c = CELL[ci];
  return [
    actTo("show", "cell_" + c.id, []),
    actTo("setText", "cap", [c.cap]), actTo("show", "cap", []),
    actTo("setText", "capsub", [c.sub]), actTo("show", "capsub", []),
    act("setState", ["cel", ci + 1]),
...
// ---------------------------------------------------------------------------
// [CELL DISMISS] a cell raises `cel` to its index and stamps `cel_t`; this
// branch pulls it down 1.6s later. One branch per cell, checked every tick.
// ---------------------------------------------------------------------------
CELL.forEach(function (c, ci) {
  ROOT.push(seq(
    cond("world.flags.cel == " + (ci + 1) + " && world.time - world.flags.cel_t > 1.6"),
    actTo("hide", "cell_" + c.id, []),
    actTo("hide", "cap", []),
    actTo("hide", "capsub", []),
    act("setState", ["cel", 0])
  ));
...
const scene = {
  kind: "kv_game_v1",
  engine: "scene",
  meta: { id: "kascity_v10", name: "KasCity", seed: "kc10", players: PLAYERS, category: "board" },
  debug: false,
  ...
  resources: {
    meshes: {
      ...
      cell_scrim: { type: "box", size: [40, 0.04, 40] },
      cell_bg: { type: "box", size: [21.5, 0.08, 11.6] },
      cell_sky: { type: "box", size: [20.6, 0.06, 5.6] },
      cell_gnd: { type: "box", size: [20.6, 0.06, 4.4] },
      cell_bar: { type: "box", size: [21.5, 0.1, 0.5] },
      cell_side: { type: "box", size: [0.5, 0.1, 11.6] },
      speed: { type: "box", size: [5.0, 0.08, 0.18] },
      fig_body: { type: "box", size: [1.15, 0.2, 2.5] },
      fig_head: { type: "box", size: [0.95, 0.2, 0.95] },
      fig_arm: { type: "box", size: [1.5, 0.2, 0.4] },
      fig_leg: { type: "box", size: [0.42, 0.2, 1.5] },
      ic_bar: { type: "box", size: [3.1, 0.16, 0.4] },
      ic_sheet: { type: "box", size: [1.9, 0.16, 3.1] },
      ic_frame: { type: "box", size: [2.5, 0.14, 3.7] },
      ic_tube: { type: "box", size: [1.9, 0.16, 0.6] },
      ic_drop: { type: "box", size: [0.5, 0.16, 0.5] },
      ic_puff: { type: "box", size: [2.2, 0.16, 1.25] },
      ic_rain: { type: "box", size: [0.24, 0.16, 1.4] },
      ic_knob: { type: "box", size: [0.42, 0.16, 0.42] },
      ic_house: { type: "box", size: [2.3, 0.16, 2.0] },
      ic_roof: { type: "box", size: [2.9, 0.16, 0.85] },
      ic_post: { type: "box", size: [0.3, 0.16, 3.0] },
      ic_flag: { type: "box", size: [1.6, 0.16, 1.0] },
      ic_case: { type: "box", size: [1.15, 0.18, 0.9] },
      ic_bench: { type: "box", size: [6.4, 0.2, 1.5] },
      ic_handle: { type: "box", size: [1.7, 0.16, 0.34] },
      ic_mallet: { type: "box", size: [1.15, 0.2, 1.0] },
...
const checks = [
  ...
  ["six comic cells", (json.match(/"cell_(deed|pipes|storm|walks|court|bust)"/g) || []).length >= 6],
  ["cell dismiss branches", (json.match(/world\.flags\.cel == [1-6]/g) || []).length === 6],
  ["cells wired to events", (json.match(/"cel",[1-6]/g) || []).length >= 6],
  ["caption labels", json.indexOf('"cap"') >= 0 && json.indexOf('"capsub"') >= 0],
  ["cells stack upward", (json.match(/"cel_scrim_/g) || []).length === 6],
  ["figures in cells", (json.match(/"fig_body"/g) || []).length >= 6],
  ["sky and ground bands", (json.match(/"cel_sky_/g) || []).length === 6 && (json.match(/"cel_gnd_/g) || []).length === 6],
  ["earth palette", json.indexOf('"paper"') >= 0 && json.indexOf('sky_court') >= 0]
];
let bad = 0;
...
fs.writeFileSync("kascity_v10.json", json);
console.log("OK kascity_v10.json (" + (json.length / 1024).toFixed(1) + " KB, BT " +
  (JSON.stringify(bt).length / 1024).toFixed(1) + " KB, nodes " + nodes.length + ")");

const engine = fs.readFileSync("scene_engine.html", "utf8");
const inject = [
  "", "// ---- injected kascity v10 showcase ----",
  "try { loadScene(" + JSON.stringify(json) + "); }",
  "catch (e) { console.error('kascity10 boot: ' + (e && e.message)); }", ""
].join("\n");
fs.writeFileSync("showcase_kascity10.html",
  engine.replace("</script>", inject + "\n</script>"));
console.log("OK showcase_kascity10.html");
