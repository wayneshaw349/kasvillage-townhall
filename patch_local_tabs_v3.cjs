// patch_local_tabs_v3.cjs — anchor edit 4 on arrow-free substrings to dodge heredoc arrow mangling.
const fs = require('fs');
...
// EDIT 2 — loader (anchor on comment line only)
...
// EDIT 4a — kill the chip render by flipping its guard to false (arrow-free anchor).
// The whole {inboxAgreements.length > 0 && ( ... )} block stays in the tree but never renders.
const OLD4a = `                {inboxAgreements.length > 0 && (
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>`;
const NEW4a = `                {false && inboxAgreements.length > 0 && (
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>`;
rep(OLD4a, NEW4a, 'kill-chips');

// EDIT 4b — inject the new tabs+list right after the (now-dead) chip block closes.
// Anchor: the "Paste Buyer Proposal" header View that follows the chips (arrow-free, unique).
const OLD4b = `                {/* PASTE BUYER PROPOSAL */}`;
const NEW4b = `                {/* LOCAL-TABS: source of truth is local_agreements. Arweave inbox still runs in the background. */}
...
rep(OLD4b, NEW4b, 'tabs-inject');

// EDIT 5 — hide Arweave inbox cards
